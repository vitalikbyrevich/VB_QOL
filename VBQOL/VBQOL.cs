namespace VBQOL
{
    ...
    class VBQOL : BaseUnityPlugin
    {
        ...
        {
            VB_FontChange.mainFontName = Config.Bind("01 - FontFix", "mainFontName", "Valheim-Norse", "Основной шрифт. Допустимые шрифты: Valheim-Norse, Valheim-Norsebold, Valheim-AveriaSansLibre, Valheim-AveriaSerifLibre, LiberationSans SDF, LiberationSans SDF - Fallback. Требуется перезапуск.");
            VB_FontChange.secondaryFontName = Config.Bind("01 - FontFix", "secondaryFontName", "Valheim-AveriaSerifLibre", "Остальной шрифт. Допустимые шрифты: Valheim-Norse, Valheim-Norsebold, Valheim-AveriaSansLibre, Valheim-AveriaSerifLibre, LiberationSans SDF, LiberationSans SDF - Fallback. Требуется перезапуск.");
            ...
            VB_GraphicPatch.streaming_mipmaps_memory_budget = Config.Bind("05 - QualitySettings", "streaming_mipmaps_memory_budget", 4096f, "Определяет сколько памяти выделено для потоковой загрузки текстур разного разрешения");
        ...
        public void ServerConfigInit()
        {
            ...
            VB_BuildDamage.enableModBDConfig = ServerConfig.BindConfig("01 - BuildDamage", "BD_Enable_Section", true, "Включите или отключите этот раздел", synced: true);
            VB_BuildDamage.creatorDamageMultConfig = ServerConfig.BindConfig("01 - BuildDamage", "BD_CreatorDamageMult", 0.75f, "Множитель урона от создателя постройки", acceptableValues: new AcceptableValueRange<float>(0f, 1f), synced: true);
            VB_BuildDamage.nonCreatorDamageMultConfig = ServerConfig.BindConfig("01 - BuildDamage", "BD_NonCreatorDamageMult", 0.05f, "Множитель урона от не создателя постройки", acceptableValues: new AcceptableValueRange<float>(0f, 1f), synced: true);
            VB_BuildDamage.uncreatedDamageMultConfig = ServerConfig.BindConfig("01 - BuildDamage", "BD_UncreatedDamageMult", 0.75f, "Множитель урона постройкам не созданным игроком", acceptableValues: new AcceptableValueRange<float>(0f, 1f), synced: true);
            VB_BuildDamage.naturalDamageMultConfig = ServerConfig.BindConfig("01 - BuildDamage", "BD_NaturalDamageMult", 0.75f, "Множитель урона от погоды и монстров.", acceptableValues: new AcceptableValueRange<float>(0f, 1f), synced: true);

            RecycleUtil.resourceMultiplier = ServerConfig.BindConfig("03 - Recycle", "R_ResourceMultiplier", 0.35f, "Количество ресурсов, возвращаемых в результате разбора (от 0 до 1, где 1 возвращает 100% ресурсов, а 0 - 0%)", synced: true);
            RecycleUtil.preserveOriginalItem = ServerConfig.BindConfig("03 - Recycle", "R_PreserveOriginalItem", true, "Сохранять ли данные оригинального предмета при понижении уровня. Полезно для модов, добавляющих дополнительные свойства к предметам, например EpicLoot.\nОтключите, если возникли проблемы.", synced: true);
        ...
        private void ApplyConfigFromPackage(ZPackage pkg)
        {
            ...
            try
            {
                ...
                VB_LeviathanPatch.m_resetLeviathanOn.Value = pkg.ReadBool();
                VB_LeviathanPatch.m_resetLeviathanLavaOn.Value = pkg.ReadBool();
                VB_LeviathanPatch.m_riseDelay.Value = pkg.ReadSingle();
        ...
        private ZPackage BuildConfigPackage()
        {
            ...
            try
            {
                pkg.Write(VB_LeviathanPatch.m_resetLeviathanOn.Value);
                pkg.Write(VB_LeviathanPatch.m_resetLeviathanLavaOn.Value);
                pkg.Write(VB_LeviathanPatch.m_riseDelay.Value);
        ...
        private void CreateConfigWatcher()
        {
            ...
            configFileWatcher.OnConfigFileReloaded += OnConfigFileWatcherOnOnConfigFileReloaded;
        ...
        private void OnConfigFileWatcherOnOnConfigFileReloaded()
        {
            if (ZNet.instance) StartCoroutine(ApplyClientConfigChanges());
        }
